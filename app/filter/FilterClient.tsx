...
export default function FilterClient() {
  ...
  const active = useMemo(() => ({ query, genre, platform }), [query, genre, platform]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="text-sm opacity-80">
        ...
        {Object.entries(active).filter(([,v]) => v).map(([k,v]) => `${k}=${v}`).join(' • ') || 'Keine'}
      </div>

      <div className="rounded-2xl border border-white/10 p-6">
        <p className="opacity-80">Hier kommen später die Ergebnisse hin.</p>
      </div>
    </div>
  );
}
